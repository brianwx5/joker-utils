...
function setupCommitLint(project) {
...
function setupDocumentation(project) {
...
function setupSemanticRelease(project) {
...
function setupPackageFields(project) {
  project.addFields({
    engines: {
      node: '>=18.0.0',
    },
...
function setupDevelopmentTools(project) {
  project.addDevDeps(
...
function setupNodeVersion(project) {
...
const project = new projen_1.typescript.TypeScriptProject({
  ...
  prettierOptions: {
    settings: {
      singleQuote: true,
      trailingComma: javascript_1.TrailingComma.ES5,
      tabWidth: 2,
      semi: true,
      printWidth: 100,
    },
